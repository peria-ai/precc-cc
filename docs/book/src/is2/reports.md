...
`precc report` býr til greiningaborð sem tekur saman PRECC-virkni og táknasparnað.
...
## Sending skýrslu með tölvupósti

Sendu skýrsluna á netfang (krefst póstuppsetningar, sjá [Email](email.md)):
...
Viðtakanetfangið er lesið úr `~/.config/precc/mail.toml`. Þú getur líka notað `precc mail report EMAIL` til að senda á ákveðið netfang.

## Skýrslugögn

Skýrslur eru búnar til úr staðbundna PRECC-gagnagrunninum í `~/.local/share/precc/history.db`. Engin gögn fara frá tölvunni þinni nema þú sendir skýrsluna sérstaklega með tölvupósti.
