...
enum Commands {
    ...
    /// Send a 90-day trial license to a user
    Trial {
        /// Email address of the trial user
        email: String,
    },
...
fn cmd_trial(email: String) -> Result<()> {
    // Restricted to authorized machines (same as license generate)
    const AUTHORIZED_FPS: &[[u8; 4]] = &[
        [0xF2, 0x9C, 0x7D, 0x98],
        [0xE5, 0x95, 0x35, 0xC6],
        [0x86, 0x29, 0x4B, 0x99],
    ...
    let expiry_days: u32 = 90;
    let now_days = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs()
        / 86400;
    let expiry_absolute = (now_days as u32) + expiry_days;

    let machine_tag = license::email_fingerprint(&email);
    let key = license::generate(machine_tag, expiry_absolute, 1); // edition_flags=1 = Pro

    let body = format!(
        "Please run the following command to install and activate precc on your Linux/Mac laptop:\n\
         \n\
         curl -fsSL https://raw.githubusercontent.com/peria-ai/precc-cc/main/scripts/install.sh | bash\n\
         \n\
         If you are using Windows, you can run this command under Powershell:\n\
         \n\
         iwr -useb https://raw.githubusercontent.com/peria-ai/precc-cc/main/scripts/install.ps1 | iex\n\
         \n\
         After that, run the following command to activate the PRO license:\n\
         \n\
         precc license activate {key} --email {email}\n\
         \n\
         Then you can get online help of the command by typing:\n\
         \n\
         precc init\n\
         \n\
         Once registered, it becomes available in Claude Code just like any built-in command.\n\
         To access additional online help, run the following command:\n\
         \n\
         precc help\n\
         \n\
         Best regards,\n\
         Precc Development Team\n"
    );

    mail::send_mail(&email, "PRECC Pro License Key", &body, &[])?;
    println!("Trial license sent to {email} (key={key}, expires in {expiry_days} days)");
    Ok(())
}
...
fn generate_precc_block() -> String {
    format!(
        "{start}\n\
         <!-- generated by precc init v{ver} — https://github.com/peria-ai/precc-cc -->\n\
         # PRECC hook\n\n\
